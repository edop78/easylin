...
export default function Users() {
  ...
  const deleteUser = async (username) => {
    if (!confirm(`Delete user "${username}" and their home directory?`)) return;
    try {
    ...
    <div className="page fade-in">
      ...
      <div className="card">
        {loading && !data ? (
            ...
            <tbody>
              {(data?.users || []).map((u) => (
                <tr key={u.uid}>
                  ...
                  <td>
                    {u.username !== 'root' && (
                      <button className="btn btn-danger btn-sm btn-icon" onClick={() => deleteUser(u.username)} title="Delete">
                        <Trash2 size={14} />
                      </button>
                    )}
                  </td>
          ...
          <table className="data-table">
            <thead>
              <tr><th>Group</th><th>GID</th><th>Members</th></tr>
            </thead>
            <tbody>
              {(groupsData?.groups || []).map((g) => (
                <tr key={g.gid}>
                  <td style={{ color: 'var(--text-primary)' }}>{g.name}</td>
                  <td className="mono">{g.gid}</td>
                  <td>{g.members || '—'}</td>
                </tr>
              ))}
            </tbody>
