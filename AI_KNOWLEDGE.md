...
## 11. Piped Installer & Shell Execution Safeguards
- **Interactive Piped Execution (`/dev/tty`)**: When executing a setup script directly from a download pipe (e.g., `wget -qO- ... | bash`), standard input (`stdin`) is occupied by the pipe. To ask interactive questions (like `(y/n)` confirmations), always check if `[ -t 0 ]` is true. If false, explicitly redirect the `read` command input from the controlling terminal device `/dev/tty` (`read ... < /dev/tty`) if it exists (`[ -c /dev/tty ]`), otherwise fallback to non-interactive defaults.
- **Root Directory Autodetection**: Piped installers run in the user's current directory (e.g. `~`), which lacks the project repository files. The installer must check if files like `docker-compose.yml` or `.env.example` exist. If missing, it must check if the `easylin` subdirectory already exists to avoid `git clone` conflicts, automatically enter the directory, or clone the remote repository and `cd` into it before running container builds.
- **Cryptographic Fallback**: When generating unique tokens or keys (`SECRET_KEY`, `JWT_SECRET`), check if `openssl` is installed. If missing, implement a fallback generation method using `/dev/urandom` and `tr`/`fold` to ensure deployment doesn't crash on bare servers.

## 12. Self-Updating Mechanism, Git Permissions, and Firewall Warnings
- **Self-Updating Dashboard Overlay**: The self-updating mechanism runs a transient systemd service unit on the host to fetch the latest changes, build, and restart the containers (`git pull && docker compose up -d --build`). During this, the frontend locks the screen with a glassmorphism progress overlay and polls the system status API, handling gateway connection failures and container recreation states gracefully.
- **Git Dubious Ownership Safeguard**: When executing Git commands on the host as `root` (via `nsenter`) inside a repository directory owned by a different non-root user (e.g. `edo`), Git blocks the execution returning exit code `128` ("dubious ownership"). Always prefix all host-level git commands with the override flag:
